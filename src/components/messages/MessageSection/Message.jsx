const Message = ({ content = "aw wchniya", time = "10:00", isSender = true, avatar = "nooo" }) => {
  return (
    <div
      className={`
        flex items-start gap-2 mb-3 animate-fadeIn
        ${isSender ? 'flex-row-reverse justify-start' : 'flex-row justify-start'}
      `}
    >
      <div className={`flex ${isSender ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        <div>
          <span
            className={`block text-xs opacity-70 mt-1 ${isSender ? "text-right" : "text-left"}`}
          >
            {time} PM
          </span>
          <div
