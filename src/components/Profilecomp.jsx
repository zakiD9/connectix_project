...
const ProfileComp = ({ isMyProfile }) => {
  const { updateProfileImage, updateBio, profileUser, showBioDialog, setshowBioDialog } = useUserStore();
  ...
  const handleFileChange = async (event) => {
    ...
    if (file) {
      const imageUrl = await apiUpdateProfileImage(file);
          ...
          {isMyProfile && (
            <>
              ...
              <span
                ...
                className="absolute bottom-0 right-0 bg-blue-500 p-1 rounded-full cursor-pointer hover:bg-blue-600 transition"
              >
